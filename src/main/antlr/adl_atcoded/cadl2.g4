...
c_complex_object: rm_type_id '[' ( ROOT_AT_CODE | AT_CODE ) ']' c_occurrences? ( SYM_MATCHES '{' c_attribute_def+ default_value? '}' )? ;
...
c_archetype_root: SYM_USE_ARCHETYPE rm_type_id '[' AT_CODE ',' archetype_ref ']' c_occurrences? ;

archetype_ref : ARCHETYPE_HRID | ARCHETYPE_REF ;

c_complex_object_proxy: SYM_USE_NODE rm_type_id '[' AT_CODE ']' c_occurrences? ADL_PATH ;

archetype_slot: SYM_ALLOW_ARCHETYPE rm_type_id '[' AT_CODE ']' (( c_occurrences? ( SYM_MATCHES '{' c_includes? c_excludes? '}' )? ) | SYM_CLOSED ) ;
...
c_regular_primitive_object: rm_type_id '[' AT_CODE ']' c_occurrences? ( SYM_MATCHES '{' c_inline_primitive_object '}' )? ;
